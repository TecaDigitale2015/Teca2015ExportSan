...
import mx.teca2015.tecaExport.tecaExportSan.scheda.UD;
...
public class TecaExportSanUD extends TecaExportSanXls {

	private Logger log = LogManager.getLogger(TecaExportSanUD.class);
	private File fXml = null;
	
	
	public TecaExportSanUD(File fXml) {
		this.fXml = fXml;
	}
	...
	public void esegui(File pathRoot) {
		...
		try {
			if (fXml.exists()){
				...
				uc = new UD(finestra.getRow(1), fXml);
...
//				} else {
//					pathFondo = pathSubFondo; 
//				}
//
//				if (uc.getFondoKey()!= null && !uc.getFondoKey().trim().equals("")){
//					pathIstituto = pathFondo.getParentFile(); 
//				}
				...
				if (uc.getAnnoIniziale() != null){
					date.put("anno iniziale", uc.getAnnoIniziale());
				}
				if (uc.getAnnoFinale() != null){
					date.put("anno finale", uc.getAnnoFinale());
				}
				if (uc.getSubFondoKey()==null || uc.getSubFondoKey().trim().equals("")){
				...
				catImport.getCatListRecords().getCatRecord().add(
						readScheda(uc.getBid(), uc.getCollocazione(), null, uc.getTitolo(), uc.getLingua(),
								date, uc.getSupporto(), null, null, null, uc.getStatoConservazione(),
								null, null, uc.getCompilatore(),uc.getDataCompilazione(), uc.getNote(), related,
				...
				Ua.genFondoEad(uc.getBid(), uc.getCollocazione(), uc.getcCol01(), uc.getcCol01Sigla(), uc.getcCol01Desc(),
						...
						uc.getcCol02Value2(), uc.getTipologiaUnitaArchivistica(), uc.getTitolo(), uc.getLingua(), 
						uc.getAnnoIniziale(), uc.getAnnoFinale(), uc.getSecoloIniziale(), uc.getSecoloFinale(), 
						uc.getSupporto(), uc.getConsistenzaCarte(), uc.getStatoConservazione(), uc.getDocumentiCartografici(), 
						uc.getChildren(), uc.getCompilatore(), uc.getDataCompilazione(), uc.getNote(), fXml.getParentFile().getAbsolutePath(), 
						uc.getBid()+"_ead");
			}
